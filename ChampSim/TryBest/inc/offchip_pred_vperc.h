#ifndef OFFCHIP_PRED_VPERC_H
#define OFFCHIP_PRED_VPERC_H

#include <unordered_set>
#include "offchip_pred_base.h"
#include "vperc_pred.h"
#include "bitmap.h"

using namespace vperc;
/* Feature set of OCP_Perc
 * Note: this feature set will be stored in LQ entry
 * and later used for training OCP */
class ocp_vperc_feature_t : public ocp_base_feature_t
{
public:
    vstate_info_t *info;
    ...
    virtual ~ocp_vperc_feature_t() // MUST be virtual
    {
        if(info) delete info;
    }
};

class ocp_vperc_page_buf_entry_t
{
public:
    uint64_t page;
    Bitmap bmp_access;
...
class OffchipPredvPerc : public OffchipPredBase
{
    private:
        vperceptron_pred_t *vperc_pred;
        vector<deque<ocp_vperc_page_buf_entry_t*> > m_page_buffer;
        ...
        vperc::vstate_info_t* get_state(ooo_model_instr *arch_instr, uint32_t data_index, LSQ_ENTRY *lq_entry);
    ...
    public:
        OffchipPredvPerc(uint32_t cpu, string _type, uint64_t _seed);
        ~OffchipPredvPerc();
