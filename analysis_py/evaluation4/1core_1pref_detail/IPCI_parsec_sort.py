...
if __name__ == "__main__":  
    for date in dates:
        ...
        for x, label in zip(x_ticks, benchs_name):
            ax.text(x, y_position, label, fontsize=6, rotation=45, ha='right', va='top')
        ...
        if(max_v < 0.5):
            ...
            y_ticks = np.around(np.arange(0.0, 2.01, 0.4), decimals=1)
            y_ticks1 = np.around(np.arange(0.0, 2.01, 0.2), decimals=1)
        ax_idx = 0
        ...
        ax.text(-1.0, 0.5, 'SpeedUp', fontsize=8, rotation=90)
        ...
        ax.legend(loc='upper center', bbox_to_anchor=(0.5, 1.30), ncol=num_prefs, fontsize=8,
            ...
            # frameon=False,
            edgecolor='none'  )
        ax.set_title('PARSEC', y=-0.55, fontsize=8) 
