...
def parse_file(file: str) -> Tuple[str, str, int, Dict[str, Any]]:
    ...
    #提取文件名称中的关键词
    match_prefetcher = re.search(r'-(.*)-no', file)
    ...
    #     return '', '', -1, {}
    if not CONFIGS["output_workloads"]:
        return '', '', -1, {}
    #提取prefetcher并判断
...
def record_results(sort_item=None):
    global berti_best 
    global hyperion_best 
    global total 
    global berti_low_higher 
    global berti_low_narrow 
    global berti_low_smaller 
    global berti_high_higher 
    global berti_high_smaller 
    res_name = CONFIGS["stats_dir"] + '/'+ CONFIGS["date"] + '.csv'
    print(res_name)
    fig_stat_name = CONFIGS["stats_dir"]+ "_figs" + '/' + CONFIGS["date"] + '.csv'

    # 获取目录路径
    res_dir1 = os.path.dirname(res_name)
    fig_dir1 = os.path.dirname(fig_stat_name)
        ...
        for trace in CONFIGS["output_workloads"] +['Average']:
            ...
            for field in CONFIGS["output_metrics"]:
                for prefetcher in sort_stat[trace] :
                    ...
                    metric_values = []
                    for trace_single, value in ROI_ORIGIN_STATS.items() :
                        if((trace+'-') in trace_single or trace == trace_single):
                            metric_values.append(ROI_ORIGIN_STATS[trace_single][prefetcher][field])
                    if(field == 'IPCI'):
                        metric_value = geo_mean(metric_values)
                    else:
                        metric_value = arith_mean(metric_values)
                    ...
                    if(field == "IPCI"):
                        ...
                        line.append(metric_value)
                    if(field == 'IPCI'):
                        count_dict[prefetcher] = metric_value
    ...
    with open(res_name, 'w') as f:
        ...
        for trace in ['Average']:
            ...
            for prefetcher in sort_stat[trace] :
                ...
                row.pop()
            row.pop()
    print('hyperion_best',hyperion_best)
    print('berti_best',berti_best)
    print('berti_high_higher',berti_high_higher)
    print('berti_high_smaller',berti_high_smaller)
    print('berti_low_higher',berti_low_higher)
    print('berti_low_narrow',berti_low_narrow)
    print('berti_low_smaller',berti_low_smaller)
