...
legends={"no":"baseline",
         "hyperion":"Hyperion",
         "hermes":"Hermes",
         "hyperion_hermes":"Hyperion_Hermes"}
...
if __name__ == "__main__":  
    for date in dates:
        ...
        data_pref = pd.read_csv(data_file_path + date + f'.csv', header=None)
        end = len(data_pref.iloc[:,1].tolist())
        data_pref = data_pref.iloc[0:end-1,:]

        # Extract the column index for "hyperion" and "LLC Coverage"
        header_row = data_pref.iloc[0]
        metric_row = data_pref.iloc[1]
        target_column_index = None
        for i, (header, metrics) in enumerate(zip(header_row, metric_row)):
            if header == "hyperion" and metrics == f'{metric}' :
                target_column_index = i
                break
        # If the target column is found, sort the data

        if target_column_index is not None:
            # Skip the header rows and sort by the target column
            sorted_data = data_pref.iloc[2:].sort_values(by=target_column_index, ascending=True)
            # Add back the header rows
            sorted_data = pd.concat([data_pref.iloc[:2], sorted_data], ignore_index=True)
        else:
            sorted_data = None
        # Display the sorted data or a message if the column wasn't found
        sorted_data.head() if sorted_data is not None else "Target column not found."
        data_pref_sorted = sorted_data.T
        # Display the transposed and sorted data
        data_pref_sorted.head()
        data_pref = data_pref_sorted  
        ...
        for bench in benchs_name:                  
            data_all[0:num_prefs,i] = get_col_data(data,metric,bench)
        ...
        left_bar_pos = np.arange(len(benchs_name))+0.3
        ## 设置图像保存的路径
        figure_res='analysis_py/evaluation4/1core_1pref_detail/'
        if not os.path.exists(figure_res):
                os.makedirs(figure_res)
        index_pref = 1
        for i in np.arange(index_pref,index_pref + 1):
            ...
            ax.bar(left_bar_pos + bar_width * (i-index_pref), 
                piece, 
                width=bar_width, 
        ...
        for x, label in zip(x_ticks, benchs_name):
            ax.text(x+0.2, y_position, label, fontsize=6, rotation=45, ha='right', va='top')
        ...
        if(max_v < 0.5):
            ...
            y_ticks1 = np.around(np.arange(0.0, 1.01, 0.1), decimals=1)
        ax_idx = 0
        ...
        ax.text(-1.6, 0.0, metric, fontsize=8, rotation=90)

        # # ax.set_title(f'IPCI', fontsize=8)
        # ax.legend(loc='upper center', bbox_to_anchor=(0.5, 1.27), ncol=num_prefs, fontsize=8,
        #     # y 参数控制标题的位置
        ...
        ax.text(1.5, 1.05, "PARSEC", fontsize=8)
        ...
        plt.savefig(f'{figure_res}/1core_1pref_{metric}_{suite}.png',dpi=800, format="png", bbox_inches='tight') 
        plt.savefig(f'{figure_res}/1core_1pref_{metric}_{suite}.pdf',dpi=800, format="pdf", bbox_inches='tight') 
        plt.savefig(f'{figure_res}/{metric}.eps',dpi=800, format="eps", bbox_inches='tight') 
        plt.close()   
